...
def load_faiss_retriever(path: str) -> FAISS:
    """
    Load a FAISS vector store from a local path (supports relative paths).
    """
...
os.makedirs(normalize_path("cases_index"), exist_ok=True)
os.makedirs(normalize_path("commercial_laws_index"), exist_ok=True)
